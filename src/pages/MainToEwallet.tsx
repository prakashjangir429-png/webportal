...
const MainWalletToEWalletTransfer = () => {
  ...
  const handleUserSelect = (userId) => {
    ...
    if (user) {
      ...
      setValue("remarks", `Transfer from Main Wallet to E-Wallet for ${user.fullName || user.userName}`);
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    if (parseFloat(data.amount) > userBalance.mainWallet) {
      toast.error(`Insufficient balance. User has ₹${userBalance.mainWallet.toFixed(2)} in Main Wallet`);
      return;
    }
    ...
    try {
      ...
      const response = await api.post("/auth/settlement/maintoEwallet", payload);
  ...
  return (
    <div>
      <PageMeta
        title="Main Wallet to E-Wallet Transfer"
        description="Transfer funds from Main Wallet to E-Wallet"
      />
      <PageBreadcrumb
        pageTitle="Main Wallet to E-Wallet Transfer"
      />
              ...
              <>
                <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        {...register("amount", {
                          ...
                          max: {
                            value: userBalance.mainWallet,
                            message: `Cannot exceed Main Wallet balance (₹${userBalance.mainWallet.toFixed(2)})`,
                          },
                          valueAsNumber: true,
                        })}
                        placeholder="0.00"
                  ...
                  <div className="mt-6 bg-gray-50 dark:bg-gray-700 p-5 rounded-lg">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className={`font-medium ${
                            projectedBalances.mainWallet < 0 
                    ...
                    {parseFloat(amount) > userBalance.mainWallet && (
                      <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 rounded-md">
                        <p className="text-red-600 dark:text-red-400 ">
                          Warning: This transfer will exceed the user's Main Wallet balance
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex justify-end">
                  <button
                    type="submit"
                    disabled={isSubmitting || !amount || parseFloat(amount) > userBalance.mainWallet}
                    className={`inline-flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 min-w-[200px] ${
                      isSubmitting || !amount || parseFloat(amount) > userBalance.mainWallet
...
export default MainWalletToEWalletTransfer;