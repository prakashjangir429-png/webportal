...
const PayInApisTable = () => {
    ...
    const fetchApis = async () => {
        try {
            ...
            const { data } = await api.get("/payIn", { params });
            setApis(data.data);
    ...
    const toggleStatus = async (id) => {
        try {
            const { data } = await api.patch(`/payIn/${id}/toggle-status`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (selectedApi) {
                // Update existing API
                await api.put(`/payIn/${selectedApi._id}`, formData);
                toast.success("API updated successfully");
            } else {
                // Create new API
                await api.post("/payIn", formData);
    ...
    return (
        <div>
            <PageMeta
                title="PayIn APIs | Admin Dashboard"
                description="Manage PayIn APIs configuration"
            />
            <PageBreadcrumb pageTitle="PayIn APIs" />
            ...
            <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-4 py-6 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 lg:px-8">
                {/* Filters Section */}
                <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-3">
                    {/* Search Filter */}
                ...
                <div className="relative w-full max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 dark:bg-gray-900">
                    <div className="mb-6">
                        <h3 className="text-xl font-semibold text-gray-800 dark:text-white/90">
                            {selectedApi ? "Edit PayIn API" : "Add New PayIn API"}
                        </h3>
                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            {selectedApi ? "Update the API details" : "Configure a new payment API"}
                        </p>
                    </div>
